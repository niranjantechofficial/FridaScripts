/* First of all its created by ApkUnpacker 
may be you guys are genius but if not then don't pretend
to be genius and stay cool and keep credit . No one is perfect in world
so if script decrypt wrong string then fix it yourself.
support all 7 string encryption of np manager , 5th is disabled by default , read there description.
and at last thanks to biN32 */ 
Java.perform(function() {
      /*1st Encryption*/
      try{ 
       
	   var ASEHC = Java.use("obfuse.NPStringFog");
	   if(ASEHC){ console.warn("APK STRING ENCRYPTION HIGH COMPATIBILITY DETECTED");}
	...
	/* 2nd Encryption */
	  ...
	  try{
	    ...
	    if(ASEHS){ console.warn("APK STRING ENCRYPTION HIGH STRENGTH DETECTED");}
      ...
      try{
	    ...
	    if(ASEHSE){ console.warn("APK STRING ENCRYPTION HIGH STRENGTH ENHANCED DETECTED");}
	   ...
	   if(ASEHCE || ASEHCE1) {  console.warn("APK STRING ENCRYPTION HIGH COMPATIBILITY ENHANCED DETECTED");}
...
try{        
	   ...
	   if(ASEE){ console.warn("APK STRING ENCRYPTION EXTREME DETECTED");}
   ...
   /* 7th encryption */
	...
	try{ 
	   ...
	   if(ASEN || ASEN2 || ASEN3){ console.warn("APK STRING ENCRYPTION NEW DETECTED");}
